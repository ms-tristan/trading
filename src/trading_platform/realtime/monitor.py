...
* :meth:`Monitor.build_result` rebuilds a
  :class:`~trading_platform.core.models.BacktestResult` from the SQLite rows
...
* :meth:`Monitor.metrics` **delegates** to
  :func:`trading_platform.metrics.compute_metrics` -- not a single formula is
  recomputed here (D6);
* :meth:`Monitor.benchmark` delegates to
  :func:`trading_platform.metrics.compare_benchmark` (or, for the
  ``random_entry`` variant, to
  :func:`trading_platform.validation.random_entry.validate_random_entry`);
* :meth:`Monitor.gate` delegates to
  :func:`trading_platform.validation.benchmark.validate_benchmark`.
...
:class:`~trading_platform.core.errors.MonitoringError`, naming the profile, so
...
from trading_platform.core.errors import MonitoringError, RealtimeError
from trading_platform.core.models import BacktestResult, TradeRecord
from trading_platform.metrics import compare_benchmark, compute_metrics
...
if TYPE_CHECKING:  # pragma: no cover - typing only, never imported at runtime
    from trading_platform.metrics import BenchmarkVariant
    from trading_platform.realtime.store import StateStore

__all__ = ["Monitor", "ProfileReport"]

#: Default benchmark variant, mirroring :data:`trading_platform.metrics.DEFAULT_BENCHMARK_VARIANT`.
...
class ProfileReport:
    ...
    :class:`~trading_platform.realtime.models.ProfileSnapshot`: the snapshot is
...
def _equity_series(points: Sequence[EquityPoint]) -> pd.Series:
    ...
    :class:`~trading_platform.core.models.BacktestResult`.  An empty input yields
...
def _resolve_benchmark_variant(explicit: str, config: RealtimeConfig | None) -> str:
    ...
    :class:`~trading_platform.config.models.RealtimeConfig` only *seeds* the
...
class Monitor:
    ...
    *reads*: it is what the CLI, the HTTP server and the tests use to render a
    ...
    ----------
    store:
        Durable state seam (``trading_platform.realtime.store.StateStore``).
    ...
    def metrics(self, profile_id: str) -> dict[str, float]:
        ...
        :func:`trading_platform.metrics.compute_metrics` returns for the rebuilt
