...
def _strategy_parameters(profile: ProfileConfig) -> dict[str, Any]:
    ...
    A plain copy of the profile's own ``params``: the strategy validates them
    with ``extra="forbid"``, so nothing is added here.
    """
    return profile.strategy_params()


def resolve_strategy(profile: ProfileConfig) -> Strategy:
    """Build the strategy of ``profile`` -- the only construction path.

    The strategy is instantiated from the profile's ``strategy`` name and its
    parameters, and validated by the strategy's own parameter model, exactly as
    the backtest engine does.
    ...
    """
    return registry.get_strategy(profile.strategy, _strategy_parameters(profile))
