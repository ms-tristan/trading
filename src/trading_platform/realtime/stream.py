...
* :class:`PollingMarketStream` -- polls a
  :class:`~trading_platform.data.loader.MarketDataProvider` through the injected
  :class:`~trading_platform.realtime.clock.Clock`;
...
def _empty_frame() -> pd.DataFrame:
    ...
    The frame satisfies :func:`~trading_platform.data.validation.ensure_ohlcv`,
...
class ReplayMarketStream:
    ...
    frames:
        ...
        copied and validated by :func:`~trading_platform.data.validation.ensure_ohlcv`.
    loop:
...
class PollingMarketStream:
    ...
    provider (:class:`~trading_platform.data.loader.CsvDataProvider`,
    ``CcxtDataProvider`` or any structural equivalent) answers a
    ``[since, until]`` window, and every frame is validated by
    :func:`~trading_platform.data.validation.ensure_ohlcv` before a candle is
