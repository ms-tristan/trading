...
Paper and live differ by three things only -- the :class:`~trading_platform.realtime.broker.Broker`
adapter injected by the orchestrator, the
:class:`~trading_platform.realtime.risk.LiveTradingGate` consulted before a ``live``
...
1. the live gate is consulted for a ``live`` profile (it may raise
   :class:`~trading_platform.core.errors.LiveTradingForbiddenError`);
2. the injected broker's ``mode`` is asserted against the profile's mode
   (the foolproof paper/live separation of D8d);
3. the per-profile :class:`~trading_platform.realtime.risk.RiskManager` verdict is
...
6. a refusal is persisted as ``REJECTED`` and raised as
   :class:`~trading_platform.core.errors.OrderRejectedError`;
...
:class:`~trading_platform.core.models.TradeRecord` covering the whole round trip.
...
from trading_platform.realtime.risk import LiveTradingGate, RiskDecision, RiskManager
