...
(:mod:`trading_platform.validation.benchmark`) answers that weaker question.  The
...
:mod:`trading_platform.metrics.random_entry` into a single boolean verdict,
...
The metrics layer (``trading_platform.metrics``) is imported **lazily** inside
...
:func:`trading_platform.validation.benchmark.validate_benchmark`.  ``risk_free_rate``
...
#: :data:`trading_platform.validation.benchmark.ComparisonFn`: it is the
...
def _resolve_entry_fn(entry_fn: RandomEntryFn | None, variant: str) -> RandomEntryFn:
    ...
    from :mod:`trading_platform.metrics.random_entry` *inside this function*, so
    ...
    try:  # lazy on purpose: the metrics layer is a downstream package
        from trading_platform.metrics.random_entry import random_entry_benchmark
    except ImportError as exc:
        raise ValidationLayerError(
            f"cannot run the {variant!r} benchmark: the metrics layer "
            f"(trading_platform.metrics) is not importable ({exc})"
        ) from exc
...
class RandomEntryGateResult:
    ...
    :class:`~trading_platform.metrics.random_entry.RandomEntryResult` so that the
    ...
    **strictly beat**.  ``strategy_beats_random`` is ``bool(p_value <
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        :meth:`trading_platform.validation.benchmark.BenchmarkGateResult.to_dict`,
...
def validate_random_entry(
    ...
    *,
    ...
    n_simulations:
        ...
        :data:`trading_platform.metrics.random_entry.DEFAULT_N_SIMULATIONS`; the
    ...
    random_seed:
        ...
        :data:`trading_platform.metrics.random_entry.DEFAULT_RANDOM_SEED`): the
    ...
    entry_fn:
        ...
        :func:`trading_platform.metrics.random_entry.random_entry_benchmark`".
