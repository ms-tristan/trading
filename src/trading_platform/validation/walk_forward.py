...
:func:`trading_platform.validation.split.make_windows` this module:
...
from trading_platform.validation.split import Window, _check_mode, make_windows
...
def _metric_call_order(metric_value: Callable[..., Any]) -> str:
    ...
    ``trading_platform.metrics`` is written by another package, and its
...
def _resolve_metric(metric: str, metric_fn: MetricFn | None) -> MetricFn:
    ...
    are imported from :mod:`trading_platform.metrics` *inside this function* so
    ...
    try:  # lazy on purpose: the metrics layer is a downstream package
        from trading_platform.metrics import METRIC_NAMES, metric_value
...
def _call_runner(
    ...
    *,
    ...
    :class:`~trading_platform.core.errors.ValidationLayerError` with the
...
def walk_forward(
    ...
    *,
    ...
    ----------
    runner:
        A :data:`~trading_platform.core.models.RunnerFn` — ``runner(frame,
