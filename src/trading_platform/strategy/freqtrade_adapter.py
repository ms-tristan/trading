...
**Layer 3** (``trading_platform.strategy``).  The dependency rule of
...
``strategy`` and not in ``trading_platform.freqtrade``: ``strategy ->
freqtrade`` is a **downward** import (layer 3 -> layer 2) and is legal, whereas
moving the adapter into ``trading_platform.freqtrade`` would be an *upward*
...
:mod:`trading_platform.strategy`, and its parameters are validated by a pydantic
...
(b) **Parameters — pydantic models vs native Freqtrade parameters.**  Delegated
    to :mod:`trading_platform.strategy.freqtrade_parameters`; this module reads
...
(c) **Stop-loss — a per-candle column vs one global attribute.**  Freqtrade has
    ...
    attribute is set to :data:`~trading_platform.strategy.freqtrade_stoploss.DEFAULT_FREQTRADE_STOPLOSS`
    ...
    :data:`~trading_platform.strategy.freqtrade_stoploss.FREQTRADE_STOPLOSS_COLUMN`
    **for audit only** — Freqtrade ignores it.  The rejected alternative (mapping
    ...
    :mod:`trading_platform.strategy.freqtrade_stoploss`.
...
from trading_platform.core.constants import DEFAULT_TIMEFRAME, SUPPORTED_TIMEFRAMES
...
#: ``TAKE_PROFIT`` (see :class:`~trading_platform.core.models.ExitReason`), so
...
def _house_strategy(self: Any) -> Strategy:
    ...
    (``_house_cache``) and rebuilt through :func:`~trading_platform.strategy.registry.get_strategy`,
    ...
    Non-mapped parameters (see :mod:`trading_platform.strategy.freqtrade_parameters`)
...
def _directional_signals(signals: pd.DataFrame, prepared: pd.DataFrame) -> pd.DataFrame:
    ...
    ``fill + (close(t) - stop_loss(t))`` (see :mod:`trading_platform.strategy.engine`).
    ``custom_stoploss`` receives a *price* of the direction, so the column has to
    be mirrored before it is mapped (this is what
    :mod:`trading_platform.strategy.freqtrade_stoploss` prescribes).
...
def _record_entry_stops(
    ...
    ``NaN`` (the ``NaN`` filter is :func:`~trading_platform.strategy.freqtrade_stoploss.entry_stop_map`'s).
    ``custom_stoploss`` later reads it back with
    :func:`~trading_platform.strategy.freqtrade_stoploss.lookup_entry_stop`, whose
...
def adapter_custom_stoploss(
    ...
    **kwargs: Any,
    ...
    float | None
        ...
        :data:`~trading_platform.strategy.freqtrade_stoploss.DEFAULT_FREQTRADE_STOPLOSS`.
...
def default_class_name(house_name: str) -> str:
    ...
    ``trading_platform.freqtrade.DEFAULT_STRATEGY_NAME`` — the name declared by
...
def freqtrade_strategy_namespace(
    ...
    *,
    ...
    :func:`~trading_platform.strategy.registry.get_strategy` — **nothing is
    hard-coded for a particular strategy**, so the factory works for every name
    of :data:`~trading_platform.strategy.registry.STRATEGIES`.  Its grid is read
    from the instance (``strategy.param_space()``) and handed explicitly to
    :func:`~trading_platform.strategy.freqtrade_parameters.freqtrade_param_specs`.

    Parameters
    ----------
    house_name:
        A key of :data:`~trading_platform.strategy.registry.STRATEGIES`.
    timeframe:
    ...
    param_overrides:
        ...
        hatch of :mod:`trading_platform.strategy.freqtrade_parameters`).  Only
