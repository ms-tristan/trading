...
``signals(prepared)``
    ...
    :data:`~trading_backtest.core.constants.SIGNAL_COLUMNS`
...
from trading_backtest.core.constants import REQUIRED_OHLCV_COLUMNS, SIGNAL_COLUMNS
...
def require_ohlcv_frame(data: pd.DataFrame, *, name: str = "data") -> pd.DataFrame:
    ...
    StrategyError
        ...
        :data:`~trading_backtest.core.constants.REQUIRED_OHLCV_COLUMNS` or holds
...
def ensure_signal_frame(signals: pd.DataFrame, index: pd.Index) -> pd.DataFrame:
    ...
    * exactly the :data:`~trading_backtest.core.constants.SIGNAL_COLUMNS`, in
...
class Strategy(ABC):
    ...
    def signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        :data:`~trading_backtest.core.constants.SIGNAL_COLUMNS`.  The method is
