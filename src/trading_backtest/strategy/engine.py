...
* fees are ``fee_rate * size * (entry_price + exit_price)``; they are recorded on
  the :class:`~trading_backtest.core.models.TradeRecord` and subtracted from its
  ``pnl``;
...
* the stop is read from the ``stop_loss`` column of the **signal candle** and
  ...
  price* with :attr:`~trading_backtest.core.models.ExitReason.STOP_LOSS`.  For a
...
* otherwise a signal exit fills at the open of the following candle
  (:attr:`~trading_backtest.core.models.ExitReason.SIGNAL`);
* a position still open on the last candle is closed at the last close
  (:attr:`~trading_backtest.core.models.ExitReason.END_OF_DATA`).
...
from trading_backtest.strategy.base import Strategy, ensure_signal_frame, require_ohlcv_frame
...
def run_backtest(
    ...
    *,
    ...
    ----------
    strategy:
        Any :class:`~trading_backtest.strategy.base.Strategy`; ``strategy.run``
        produces the prepared frame and the signals.
    data:
        OHLCV frame satisfying the contract (see
        :func:`~trading_backtest.strategy.base.require_ohlcv_frame`).  It is never
        mutated.
    initial_balance:
...
def make_runner(cfg: AppConfig, *, symbol: str | None = None) -> RunnerFn:
    """Return the :data:`~trading_backtest.core.models.RunnerFn` bound to ``cfg``.
