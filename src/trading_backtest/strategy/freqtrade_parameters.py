...
**Layer 3** (``trading_backtest.strategy``): this module imports
:mod:`trading_backtest.core` and :mod:`trading_backtest.strategy` only — never
``config``, ``data``, ``metrics``, ``reporting``, ``validation`` or ``cli``.
``freqtrade`` is an **optional extra** (``trading-backtest[freqtrade]``) that is
...
in :mod:`trading_backtest.strategy` and their parameters are validated by a
pydantic :class:`~trading_backtest.strategy.base.StrategyParams` model.  Freqtrade
...
def freqtrade_param_specs(
    ...
    *,
    ...
    params_model:
        ...
        :class:`~trading_backtest.strategy.base.Strategy` subclass, or a
        :class:`~trading_backtest.strategy.base.Strategy` **instance** — in which
...
def freqtrade_parameter_attributes(specs: Sequence[FreqtradeParamSpec]) -> dict[str, Any]:
    ...
    :mod:`trading_backtest.strategy.freqtrade_adapter`).  Insertion order
    follows ``specs``.
