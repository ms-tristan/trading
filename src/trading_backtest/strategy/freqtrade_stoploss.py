...
2. **captures the absolute stop of every candle** in a
   ...
   the house engine (see :mod:`trading_backtest.strategy.engine`);
...
from trading_backtest.core.constants import candle_delta
...
#: :mod:`trading_backtest.strategy.freqtrade_parameters` and
#: :mod:`trading_backtest.strategy.freqtrade_adapter`, so a caller catching the
...
def candle_floor(timestamp: Any, timeframe: str) -> pd.Timestamp:
    ...
    floored with :func:`~trading_backtest.core.constants.candle_delta`.
...
def entry_stop_map(signals: pd.DataFrame) -> dict[pd.Timestamp, float]:
    ...
    signals:
        ...
        :data:`~trading_backtest.core.constants.SIGNAL_COLUMNS`), indexed by the
