...
request into one :meth:`~trading_backtest.web.routes.Router.handle` call and
writes the resulting :class:`~trading_backtest.web.routes.HttpResponse` back.
...
* ``log_message``/``log_error`` are routed to the ``trading_backtest.web``
...
class MonitoringHandler(BaseHTTPRequestHandler):
    ...
    server_version = "trading-backtest-monitoring/0.1"
    ...
    def log_message(self, format: str, *args: Any) -> None:  # noqa: A002 - stdlib API
        """Log one request line to ``trading_backtest.web`` instead of stderr."""
...
def start_in_thread(
    ...
    thread = threading.Thread(
        ...
        name="trading-backtest-monitoring",
        daemon=True,
    )
    thread.start()
    return thread
