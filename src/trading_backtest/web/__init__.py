...
Public surface
--------------
* :class:`~trading_backtest.web.routes.Router` -- the pure request/response
  mapping (no socket, no thread), and the whole JSON contract of the monitoring
  API;
* :class:`~trading_backtest.web.routes.SnapshotProvider` -- the *only* seam the
...
* :class:`~trading_backtest.web.server.MonitoringServer` plus
  :func:`~trading_backtest.web.server.create_server`,
  :func:`~trading_backtest.web.server.serve` and
  :func:`~trading_backtest.web.server.start_in_thread` -- the HTTP transport;
* :func:`~trading_backtest.web.routes.operator_token_from_env` -- the single
...
from trading_backtest.web.routes import Router, SnapshotProvider, operator_token_from_env
