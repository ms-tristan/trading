...
paper-or-live -- over a :class:`~trading_backtest.realtime.stream.MarketStream`.
...
entry/exit rules come from :meth:`trading_backtest.strategy.base.Strategy.run`
(the single entry point), metrics come from the metrics layer, OHLCV hygiene comes
from :func:`trading_backtest.data.validation.ensure_ohlcv`, and the whole order
lifecycle (risk, idempotency, persistence, reconciliation) belongs to the
:class:`~trading_backtest.realtime.gateway.ExecutionGateway`.  What is left here is
...
from trading_backtest.core.models import Direction, ExitReason
...
from trading_backtest.realtime.observability import LOGGER_NAME, Counters, log_event
...
class ProfileRunner:
    ...
    async def start(self) -> None:
        ...
        degraded = self._status is ProfileStatus.DEGRADED
