...
secret redaction
    ...
    ones ``trading_backtest.realtime.credentials`` reads; the eight lines are
...
in-process counters
    ...
    :class:`~trading_backtest.realtime.models.EngineCounters`, exposed through the
...
:class:`~trading_backtest.realtime.clock.SystemClock`, which is the single owner of
"now" in this layer.
"""
...
LOGGER_NAME = "trading_backtest.realtime"
...
_HANDLER_FLAG = "_trading_backtest_realtime_handler"
...
def _environment_secrets(environ: Mapping[str, str]) -> tuple[str, ...]:
    ...
    The scan mirrors ``trading_backtest.realtime.credentials``: the three global
...
def log_path(logs_dir: Path, *, moment: datetime | None = None) -> Path:
    ...
    moment:
        ...
        through :class:`~trading_backtest.realtime.clock.SystemClock`.
    """
