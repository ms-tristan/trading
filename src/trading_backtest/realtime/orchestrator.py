...
state store, the global kill switch and one :class:`~trading_backtest.realtime.runner.ProfileRunner`
...
``stream_factory``
    builds the :class:`~trading_backtest.realtime.stream.MarketStream` of a profile.
    ...
    :class:`~trading_backtest.core.errors.MarketStreamError` instead of silently
    inventing one.
``broker_factory``
    builds the venue adapter; it defaults to :func:`default_broker_factory`, which
    picks :class:`~trading_backtest.realtime.broker.PaperBroker` for a paper profile
    and :class:`~trading_backtest.realtime.broker.CcxtBroker` for a live one.  The
...
:class:`~trading_backtest.realtime.gateway.ExecutionGateway`.  The mechanical
...
from trading_backtest.config.models import MonitoringConfig, ProfileConfig, RealtimeConfig
from trading_backtest.core.errors import MarketStreamError, ProfileError, RealtimeError
...
from trading_backtest.realtime.observability import LOGGER_NAME, log_event
...
if TYPE_CHECKING:
    ...
    from trading_backtest.realtime.stream import MarketStream
...
def make_risk_manager(
    ...
    """
    from trading_backtest.realtime.risk import RiskLimits, RiskManager
...
def make_live_gate(environ: Mapping[str, str] | None = None) -> LiveTradingGate:
    ...
    """
    from trading_backtest.realtime.risk import LiveTradingGate
...
def default_broker_factory(
    ...
    *,
    ...
    A ``paper`` profile always gets a :class:`~trading_backtest.realtime.broker.PaperBroker`
    ...
    :class:`~trading_backtest.realtime.broker.CcxtBroker` built from the credentials
    ...
    if str(profile.mode) == RunMode.LIVE.value:
        from trading_backtest.realtime.broker import CcxtBroker
...
class RealtimeOrchestrator:
    ...
    profiles:
        ...
        :class:`~trading_backtest.core.errors.ProfileError`: a platform that
